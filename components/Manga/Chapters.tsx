...
import { EpisodeCard } from '../Anime/EpisodeCard'

export const Chapters = ({ id, cover }: any) => {
  ...
  const getMovies = async () => {
    try {
      ...
      const animes = await kitsu.getMangaChapters(id)
      setData(animes.data)
        ...
        <>
          ...
          <View style={styles.cont}>
            {data.map((item: any, i: number) => (
              <EpisodeCard type={'chapter'} key={i} cover={cover} xdata={item.attributes} anime={id} id={item.id} />
            ))}
            {next ? (
              <Button
                onPress={async () => {
